...
public class TeamQuery {
    ...
    public static Team getTeamById(int id) throws TeamNotFoundExeption {
        DSLContext ctx = JooqContextProvider.getDSLContext();
        Result<org.jooq.Record> result = ctx.select().from(TEAM).leftJoin(PLAYER).on(PLAYER.TEAM_ID.eq(TEAM.ID)).where(TEAM.ID.eq(id)).fetch();
        Team team = null;
        for(org.jooq.Record r : result){
            if(team == null) {
                team = new Team(r.get(TEAM.ID), r.get(TEAM.NAME));
            }
            PlayerQuery.extractPlayerFromRecord(r).ifPresent(team::addPlayer);
        }
        return team;
    }
